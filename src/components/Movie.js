...
const Movie = ({ movie }) => {
  ...
  return (
    <>
      <section className="movie" id={movie.imdbID} onClick={() => handleClickShowModal(movie.imdbID) }>
        <img className="movie__image" src={movie.Poster} alt="movie's poster"/>
        <h2 className="movie__title">{movie.Title}</h2>
        <p className="movie__category">{movie.Genre}</p>
        <p className="movie__rating"><AiFillHeart className="icons"/>{ ratings===0 ? null : movie.Ratings[0].Value }</p>
      </section>
